...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		...
		-- ── SubOptions panel ──────────────────────────────────────────────────
		if Config.SubOptions and #Config.SubOptions > 0 then
			-- The BasedFrame is Handler's parent's parent chain — we find it via
			-- Toggle's parent (Handler) parent (BasedFrame).
			local BasedFrame = Handler.Parent;
			local ROW_H      = 30;
			local ITEM_H     = 26;
			local PAD_V      = 6;
			local panelH     = #Config.SubOptions * ITEM_H + PAD_V * 2;
			local expanded   = false;

			-- chevron button sits in Handler (same row as toggle)
			local ChevBtn    = Instance.new("Frame");
			local ChevIcon   = Instance.new("TextLabel");
			local ChevCorner = Instance.new("UICorner");

			ChevBtn.Name               = NeverLose.RandomString();
			ChevBtn.Parent             = Handler;
			ChevBtn.BackgroundColor3   = Color3.fromRGB(26, 28, 36);
			ChevBtn.BackgroundTransparency = 0.4;
			ChevBtn.BorderSizePixel    = 0;
			ChevBtn.Size               = UDim2.new(0, 18, 0, 18);
			ChevBtn.ZIndex             = ZINdex + 13;
			ChevBtn.LayoutOrder        = -(#Handler:GetChildren() + 5);
			ChevCorner.CornerRadius    = UDim.new(0, 4);
			ChevCorner.Parent          = ChevBtn;

			ChevIcon.Name              = NeverLose.RandomString();
			ChevIcon.Parent            = ChevBtn;
			ChevIcon.AnchorPoint       = Vector2.new(0.5, 0.5);
			ChevIcon.BackgroundTransparency = 1;
			ChevIcon.BorderSizePixel   = 0;
			ChevIcon.Position          = UDim2.new(0.5, 0, 0.5, 0);
			ChevIcon.Size              = UDim2.new(1, 0, 1, 0);
			ChevIcon.ZIndex            = ZINdex + 14;
			ChevIcon.FontFace          = NeverLose.BuiltInBold;
			ChevIcon.Text              = "chevron-small-right";
			ChevIcon.TextColor3        = Color3.fromRGB(180, 180, 180);
			ChevIcon.TextSize          = 14;
			ChevIcon.TextTransparency  = 0.3;
			ChevIcon.TextWrapped       = true;

			-- sub panel frame sits below the row inside BasedFrame
			...
			SubPanel.Name              = NeverLose.RandomString();
			SubPanel.Parent            = BasedFrame;
			SubPanel.BackgroundColor3  = Color3.fromRGB(12, 13, 20);
			SubPanel.BackgroundTransparency = 0;
			SubPanel.BorderSizePixel   = 0;
			SubPanel.ClipsDescendants  = true;
			SubPanel.Position          = UDim2.new(0, 0, 0, ROW_H);
			SubPanel.Size              = UDim2.new(1, 0, 0, 0);  -- collapsed
			SubPanel.ZIndex            = ZINdex + 8;
			SubPanel.Visible           = false;

			SubCorner.CornerRadius     = UDim.new(0, 8);
			SubCorner.Parent           = SubPanel;

			SubStroke.Color            = Color3.fromRGB(45, 48, 58);
			SubStroke.Transparency     = 0.5;
			SubStroke.Parent           = SubPanel;

			SubList.Parent             = SubPanel;
			...
			SubPad.Parent              = SubPanel;
			SubPad.PaddingLeft         = UDim.new(0, 10);
			SubPad.PaddingRight        = UDim.new(0, 10);
			SubPad.PaddingTop          = UDim.new(0, PAD_V);
			SubPad.PaddingBottom       = UDim.new(0, PAD_V);

			-- track sub-option state
			local SubState = {};

			for si, opt in next, Config.SubOptions do
				opt.Default = (opt.Default ~= false);  -- default true if not set
				SubState[si] = { Value = opt.Default, Color = opt.Color or Color3.fromRGB(160,160,160) };
				...
				ItemRow.Name              = NeverLose.RandomString();
				ItemRow.Parent            = SubPanel;
				ItemRow.BackgroundTransparency = 1;
				ItemRow.BorderSizePixel   = 0;
				ItemRow.Size              = UDim2.new(1, 0, 0, ITEM_H);
				ItemRow.ZIndex            = ZINdex + 9;

				CheckBox.Name             = NeverLose.RandomString();
				CheckBox.Parent           = ItemRow;
				CheckBox.AnchorPoint      = Vector2.new(0, 0.5);
				CheckBox.Position         = UDim2.new(0, 0, 0.5, 0);
				CheckBox.Size             = UDim2.new(0, 13, 0, 13);
				CheckBox.BackgroundColor3 = opt.Default and NeverLose.AccentColor or Color3.fromRGB(22, 24, 34);
				CheckBox.BackgroundTransparency = 0;
				CheckBox.BorderSizePixel  = 0;
				CheckBox.ZIndex           = ZINdex + 10;
				CheckCorner.CornerRadius  = UDim.new(0, 3);
				CheckCorner.Parent        = CheckBox;

				local CheckStroke = Instance.new("UIStroke");
				CheckStroke.Color         = Color3.fromRGB(55, 58, 72);
				CheckStroke.Transparency  = opt.Default and 1 or 0.3;
				...
				CheckMark.ZIndex          = ZINdex + 11;
				CheckMark.FontFace        = NeverLose.BuiltInBold;
				CheckMark.Text            = "check-small";
				CheckMark.TextColor3      = Color3.fromRGB(255, 255, 255);
				CheckMark.TextSize        = 12;
				CheckMark.TextTransparency = opt.Default and 0 or 1;
				...
				ItemLabel.Position        = UDim2.new(0, 20, 0.5, 0);
				ItemLabel.Size            = UDim2.new(1, -42, 0, 14);
				ItemLabel.BackgroundTransparency = 1;
				ItemLabel.BorderSizePixel = 0;
				ItemLabel.ZIndex          = ZINdex + 10;
				ItemLabel.Font            = Enum.Font.GothamMedium;
				ItemLabel.Text            = opt.Name or ("Option "..si);
				ItemLabel.TextColor3      = Color3.fromRGB(200, 200, 200);
				ItemLabel.TextSize        = 11;
				ItemLabel.TextTransparency = 0.25;
				ItemLabel.TextXAlignment  = Enum.TextXAlignment.Left;

				ColorDot.Name             = NeverLose.RandomString();
				ColorDot.Parent           = ItemRow;
				ColorDot.AnchorPoint      = Vector2.new(1, 0.5);
				ColorDot.Position         = UDim2.new(1, 0, 0.5, 0);
				ColorDot.Size             = UDim2.new(0, 13, 0, 13);
				ColorDot.BackgroundColor3 = SubState[si].Color;
				ColorDot.BackgroundTransparency = 0;
				ColorDot.BorderSizePixel  = 0;
				ColorDot.ZIndex           = ZINdex + 10;
				ColorCorner.CornerRadius  = UDim.new(0, 3);
				ColorCorner.Parent        = ColorDot;
				ColorStroke.Color         = Color3.fromRGB(255,255,255);
				ColorStroke.Transparency  = 0.85;
				ColorStroke.Parent        = ColorDot;

				-- checkbox click
				local function UpdateCheck(s)
					SubState[si].Value = s;
					NeverLose.PlayAnimate(CheckBox, SlowyTween, {
						BackgroundColor3 = s and NeverLose.AccentColor or Color3.fromRGB(22, 24, 34)
					});
					NeverLose.PlayAnimate(CheckMark, SlowyTween, {
						TextTransparency = s and 0 or 1
					});
					NeverLose.PlayAnimate(CheckStroke, SlowyTween, {
						Transparency = s and 1 or 0.3
					});
					Config.Callback(Config.Default, SubState);
				end;

				NeverLose:CreateInput(ItemRow, LPH_NO_VIRTUALIZE(function()
					UpdateCheck(not SubState[si].Value);
				end));

				-- color dot opens a color picker
				local colorSignal;
				local BackendColor = NeverLose:CreateColorPicker(ColorDot);
				BackendColor:SetValue(SubState[si].Color);
				BackendColor.Callback = function(c)
					SubState[si].Color = c;
					ColorDot.BackgroundColor3 = c;
					Config.Callback(Config.Default, SubState);
				end;

				NeverLose:CreateInput(ColorDot, LPH_NO_VIRTUALIZE(function()
					if colorSignal then colorSignal:Disconnect(); colorSignal = nil; end;
					BackendColor.SetRender(true);
					colorSignal = UserInputService.InputBegan:Connect(function(Input)
						if Input.UserInputType == Enum.UserInputType.MouseButton1 then
							if not NeverLose:IsMouseOverFrame(ColorDot) and not NeverLose:IsMouseOverFrame(BackendColor.Root) then
								if colorSignal then colorSignal:Disconnect(); colorSignal = nil; end;
								BackendColor.SetRender(false);
							end;
						end;
					end);
				end));
			...
			local function SetExpanded(val)
				...
				if val then
					...
					NeverLose.PlayAnimate(SubPanel, SlowyTween, {
						Size = UDim2.new(1, 0, 0, panelH)
					});
					NeverLose.PlayAnimate(BasedFrame, SlowyTween, {
						Size = UDim2.new(1, 0, 0, ROW_H + panelH + 4)
					});
					NeverLose.PlayAnimate(ChevIcon, SlowyTween, {
						TextColor3 = NeverLose.AccentColor,
						Rotation = 90
					});
					NeverLose.PlayAnimate(ChevBtn, SlowyTween, {
					...
					task.delay(SlowyTween.Time + 0.05, function()
						if not expanded then SubPanel.Visible = false; end;
					end);
				end;
			end;

			NeverLose:CreateInput(ChevBtn, LPH_NO_VIRTUALIZE(function()
				SetExpanded(not expanded);
			end));

			-- expose on ToggleLib
			function ToggleLib:GetSubOptions()
				return SubState;
			end;

			function ToggleLib:SetExpanded(val)
				SetExpanded(val);
			end;
		end;
		-- ── end SubOptions ────────────────────────────────────────────────────
...
function NeverLose:CreateWindow(Config)
	...
	function Window:AddTab(Config)
		...
		-- ── SubTab / Page system ────────────────────────────────────────────────
		Tab.Pages       = {};   -- list of { LeftScroll, RightScroll, Button, BtnLabel, BtnLine }
		Tab.CurrentPage = 1;

		-- Subtab bar: a thin horizontal strip at the top of TabFrame, hidden until
		-- at least one page is added via AddPage.
		local SubTabBar = Instance.new("Frame")
		local SubTabList = Instance.new("UIListLayout")
		local SubTabPad  = Instance.new("UIPadding")
		local SubTabSep  = Instance.new("Frame")

		SubTabBar.Name               = NeverLose.RandomString();
		...
		SubTabBar.Size               = UDim2.new(1, 0, 0, 0);   -- collapsed until pages exist
		...
		SubTabPad.PaddingLeft        = UDim.new(0, 10);
		SubTabPad.PaddingTop         = UDim.new(0, 4);
		SubTabPad.PaddingBottom      = UDim.new(0, 4);

		SubTabSep.Name               = NeverLose.RandomString();
		SubTabSep.Parent             = TabFrame;
		SubTabSep.AnchorPoint        = Vector2.new(0.5, 0);
		SubTabSep.BackgroundColor3   = Color3.fromRGB(45, 48, 58);
		SubTabSep.BackgroundTransparency = 1;   -- hidden until pages exist
		SubTabSep.BorderSizePixel    = 0;
		SubTabSep.Position           = UDim2.new(0.5, 0, 0, 0);
		SubTabSep.Size               = UDim2.new(1, -10, 0, 1);
		SubTabSep.ZIndex             = 10;

		Tab.SubTabBar = SubTabBar;

		-- Internal: switch active page, tween button states
		local function SetActivePage(idx)
			Tab.CurrentPage = idx;
			for i, pg in next, Tab.Pages do
				local isActive = (i == idx);
				-- show / hide scroll columns
				pg.LeftScroll.Visible  = isActive;
				pg.RightScroll.Visible = isActive;
				-- button active state tween
				if isActive then
					NeverLose.PlayAnimate(pg.Button, SlowyTween, {
						BackgroundTransparency = 0.15,
						BackgroundColor3 = NeverLose.AccentColor
					});
					NeverLose.PlayAnimate(pg.BtnLabel, SlowyTween, {
						TextTransparency = 0,
		...
		-- Expose so external code can switch pages programmatically
		function Tab:SetPage(idx)
			if Tab.Pages[idx] then
				SetActivePage(idx);
			end;
		end;

		-- Add a named sub-page.  Returns a page handle with :AddSection().
		-- The very first AddPage call also repositions the existing LeftScroll /
		-- RightScroll so they sit below the subtab bar.
		function Tab:AddPage(Config)
			Config = NeverLose:ProcessParams(Config, {
				Name = "Page",
			});

			local pageIdx = #Tab.Pages + 1;
			local BAR_H   = 30;  -- subtab bar height

			-- ── on first page: reveal bar, reposition existing scrolls ──────────
			if pageIdx == 1 then
				SubTabBar.Size = UDim2.new(1, 0, 0, BAR_H);
				NeverLose.PlayAnimate(SubTabSep, SlowyTween, {
					BackgroundTransparency = 0.650
				});
				SubTabSep.Position = UDim2.new(0.5, 0, 0, BAR_H);

				-- Shift the existing twin-column scrolls down
				LeftScroll.Position  = UDim2.new(0.25, 0, 0.5, BAR_H / 2);
				LeftScroll.Size      = UDim2.new(0.5, 0, 1, -(5 + BAR_H));
				RightScroll.Position = UDim2.new(0.75, 0, 0.5, BAR_H / 2);
				RightScroll.Size     = UDim2.new(0.5, 0, 1, -(5 + BAR_H));

				-- If Single-type tab was used, fix its single scroll too
				if LeftScroll == RightScroll then
					LeftScroll.Position = UDim2.new(0.5, 0, 0.5, BAR_H / 2);
					LeftScroll.Size     = UDim2.new(1, 0, 1, -(5 + BAR_H));
				end;
			end;

			-- ── build the page's own scroll pair ────────────────────────────────
			local pgLeft, pgRight, pgListL, pgListR;

			if Config.Type == "Single" then
				pgLeft = Instance.new("ScrollingFrame");
				pgLeft.Name               = NeverLose.RandomString();
				pgLeft.Parent             = TabFrame;
				pgLeft.Active             = true;
				pgLeft.AnchorPoint        = Vector2.new(0.5, 0.5);
				pgLeft.BackgroundTransparency = 1;
				pgLeft.BorderSizePixel    = 0;
				pgLeft.ClipsDescendants   = false;
				pgLeft.Position           = UDim2.new(0.5, 0, 0.5, BAR_H / 2);
				pgLeft.Size               = UDim2.new(1, 0, 1, -(5 + BAR_H));
				pgLeft.ScrollBarThickness = 0;
				pgLeft.Visible            = false;
				pgListL = Instance.new("UIListLayout");
				pgListL.Parent            = pgLeft;
				pgListL.SortOrder         = Enum.SortOrder.LayoutOrder;
				pgListL.Padding           = UDim.new(0, 5);
				NeverLose:AddSignal(pgListL:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
					pgLeft.CanvasSize = UDim2.fromOffset(0, pgListL.AbsoluteContentSize.Y + 1);
				end)));
				pgRight   = pgLeft;
				pgListR   = pgListL;
			else
				...
				pgLeft.BorderSizePixel    = 0;
				pgLeft.ClipsDescendants   = false;
				pgLeft.Position           = UDim2.new(0.25, 0, 0.5, BAR_H / 2);
				pgLeft.Size               = UDim2.new(0.5, 0, 1, -(5 + BAR_H));
				pgLeft.ScrollBarThickness = 0;
				pgLeft.Visible            = false;
				pgListL = Instance.new("UIListLayout");
				pgListL.Parent            = pgLeft;
				pgListL.HorizontalAlignment = Enum.HorizontalAlignment.Right;
				pgListL.SortOrder         = Enum.SortOrder.LayoutOrder;
				pgListL.Padding           = UDim.new(0, 5);
				NeverLose:AddSignal(pgListL:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
					pgLeft.CanvasSize = UDim2.fromOffset(0, pgListL.AbsoluteContentSize.Y + 1);
				...
				pgRight = Instance.new("ScrollingFrame");
				pgRight.Name              = NeverLose.RandomString();
				pgRight.Parent            = TabFrame;
				pgRight.Active            = true;
				pgRight.AnchorPoint       = Vector2.new(0.5, 0.5);
				pgRight.BackgroundTransparency = 1;
				pgRight.BorderSizePixel   = 0;
				pgRight.ClipsDescendants  = false;
				pgRight.Position          = UDim2.new(0.75, 0, 0.5, BAR_H / 2);
				pgRight.Size              = UDim2.new(0.5, 0, 1, -(5 + BAR_H));
				pgRight.ScrollBarThickness = 0;
				pgRight.Visible           = false;
				pgListR = Instance.new("UIListLayout");
				pgListR.Parent            = pgRight;
				pgListR.SortOrder         = Enum.SortOrder.LayoutOrder;
				pgListR.Padding           = UDim.new(0, 5);
				NeverLose:AddSignal(pgListR:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
					pgRight.CanvasSize = UDim2.fromOffset(0, pgListR.AbsoluteContentSize.Y + 1);
				end)));
			end;

			-- ── subtab pill button ───────────────────────────────────────────────
			-- measure text width so the frame has a fixed, correct pixel size
			local btnTextSize = TextService:GetTextSize(
				Config.Name, 11, Enum.Font.GothamMedium, Vector2.new(math.huge, math.huge)
			);
			local btnW = btnTextSize.X + 20; -- 10px padding each side

			local BtnFrame  = Instance.new("Frame");
			local BtnCorner = Instance.new("UICorner");
			local BtnLabel  = Instance.new("TextLabel");

			BtnFrame.Name               = NeverLose.RandomString();
			BtnFrame.Parent             = SubTabBar;
			BtnFrame.BackgroundColor3   = Color3.fromRGB(30, 32, 40);
			BtnFrame.BackgroundTransparency = 0.80;
			BtnFrame.BorderSizePixel    = 0;
			BtnFrame.Size               = UDim2.new(0, btnW, 0, 22);
			...
			BtnLabel.Name               = NeverLose.RandomString();
			...
			BtnLabel.TextTransparency   = 0.45;
			...
			table.insert(Tab.Pages, pageEntry);

			-- single input for both click and hover (no double-stacking)
			local btnInput, _ = NeverLose:CreateInput(BtnFrame, LPH_NO_VIRTUALIZE(function()
				SetActivePage(pageIdx);
			end));

			NeverLose:AddSignal(btnInput.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
				if Tab.CurrentPage ~= pageIdx then
					NeverLose.PlayAnimate(BtnFrame, SlowyTween, {
			...
			-- ── page object returned to caller ───────────────────────────────────
			local Page = {};

			function Page:AddSection(PageConfig)
				PageConfig = NeverLose:ProcessParams(PageConfig, {
					Name     = "SECTION",
					Position = 'left'
				});
				...
				local UIStrokePg     = Instance.new("UIStroke")
				local UICornerPg     = Instance.new("UICorner")
				local UIListLayoutPg = Instance.new("UIListLayout")

				SectionFrame.Name               = NeverLose.RandomString();
				SectionFrame.Parent             = (string.lower(PageConfig.Position) == 'left' and pgLeft) or pgRight;
				...
				SectionLabel.Text               = PageConfig.Name;
				...
				UIStrokePg.Transparency         = 0.650;
				UIStrokePg.Color                = Color3.fromRGB(45, 48, 58);
				UIStrokePg.Parent               = SectionHandler;

				UICornerPg.CornerRadius         = UDim.new(0, 10);
				UICornerPg.Parent               = SectionHandler;

				UIListLayoutPg.Parent           = SectionHandler;
				UIListLayoutPg.HorizontalAlignment = Enum.HorizontalAlignment.Center;
				UIListLayoutPg.SortOrder        = Enum.SortOrder.LayoutOrder;

				UIListLayoutPg:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(LPH_NO_VIRTUALIZE(function()
					if UIListLayoutPg.AbsoluteContentSize.Y <= 1 then
						NeverLose.PlayAnimate(SectionFrame, VSlowTween, {
							Size = UDim2.new(1, -5, 0, 0)
						});
					else
						NeverLose.PlayAnimate(SectionFrame, VSlowTween, {
							Size = UDim2.new(1, -5, 0, UIListLayoutPg.AbsoluteContentSize.Y + 19.5)
						});
					end;
				end));
				...
				Section.SetRender = LPH_NO_VIRTUALIZE(function(value)
					if value then
						...
						NeverLose.PlayAnimate(UIStrokePg, SlowyTween, { Transparency = 0.650 });
					else
						...
						NeverLose.PlayAnimate(UIStrokePg, SlowyTween, { Transparency = 1 });
					end;
				end);
		...
		function Tab:AddSection(Config)
			...
			-- If pages exist, route to the active page's scrolls instead
			local _LeftScroll  = LeftScroll;
			local _RightScroll = RightScroll;
			if #Tab.Pages > 0 then
				local pg = Tab.Pages[Tab.CurrentPage] or Tab.Pages[1];
				_LeftScroll  = pg.LeftScroll;
				_RightScroll = pg.RightScroll;
			end;

			SectionFrame.Name = NeverLose.RandomString();
			SectionFrame.Parent = (string.lower(Config.Position) == 'left' and _LeftScroll) or _RightScroll
			SectionFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
